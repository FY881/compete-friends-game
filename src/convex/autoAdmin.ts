...
async function performSweep(ctx: {
  ...
  if (openReports.length > 0 && !apiKey) {
    issues.push({
      severity: "high",
      title: "لا يوجد نظام API مُفعّل — لا يمكن فحص البلاغات",
      detail: `يوجد ${openReports.length} بلاغ مفتوح بانتظار مراجعة الذكاء الاصطناعي لكن لا يوجد نظام مُفعّل في مركز API.`,
      fix: "فعّل النظام الأول (مفتاح + رابط) أو النظام الثاني (مفتاح فقط) من تبويب «مركز API» في غرفة المالك، واضغط «تحقق» للتأكد من صلاحية الربط.",
    });
  } else {
  ...
  if (!apiKey) {
    ...
    if (openReports.length === 0) {
      issues.push({
        severity: "high",
        title: "لا يوجد نظام API مُفعّل",
        detail: "المدير الآلي يعمل لكنه لا يستطيع مراجعة البلاغات بالذكاء الاصطناعي دون نظام مُفعّل.",
        fix: "فعّل النظام الأول (مفتاح + رابط) أو النظام الثاني (مفتاح فقط) في تبويب «مركز API» بغرفة المالك، ثم اضغط «تحقق» لتأكيد الربط.",
      });
    }
  }
